...
class Element:
    ...
    def update(self):
        ...
        This method may be expanded or overridden by subclasses to implement more specific
        behaviour at either the abstraction level (e.g. Liquid, Gas) or the
        individual element level (e.g. Water, Smoke).
        """

        if self.has_been_updated:
            return # if we've already updated this step, skip

        row, col = self.location
        
        # check below
        if (target := self.grid.get_pos(row+1, col)): # if the target isn't None
...
class Liquid(Element):
    ...
    def update(self):
        super().update() # use default update behaviour first
        # if the liquid still hasn't been updated this step, we'll use the expanded logic
...
class Gas(Element):
    ...
    def update(self):
        super().update() # use default update behaviour first
        # if the gas still hasn't been updated this step, we'll use the expanded logic
...
print("Using new particles structure")