...
class ParticleSim:
    ...
    def is_empty(self, row: int, col: int) -> bool:
        ...
        # a location is considered empty if occupied by Void
        return self.matrix[row][col].ID == Void.ID
    ...
    def update(self):
        ...
        for dest_row, dest_col in checks:
            if (target := self.grid.get_pos(dest_row, dest_col)):
                # check if the target is Void, as this is always a valid target
                if target.ID == Void.ID:
                    self.grid.swap(self, target)
                    return

                # rule out invalid targets quickly
                if not target.MOVABLE or target.has_been_updated:
                    continue # skip this target
                
                if self.grid.can_move_density(self, target):
                    self.grid.swap(self, target)
                    return
...
class Gas(Element):
    ...
    def update(self):
        ...
        for dest_row, dest_col in checks:
            if (target := self.grid.get_pos(dest_row, dest_col)):
                # check if the target is Void, as this is always a valid space
                if target.ID == Void.ID:
                    self.grid.swap(self, target)
                    return
